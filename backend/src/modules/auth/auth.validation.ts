...
export const resetPasswordSchema = z.object({ token: z.string().min(32), newPassword: z.string().min(8).max(128) });
export const updateProfileSchema = z.object({ fullName: z.string().min(2).max(120) });
